...
def check_network(networks,controller_node,ex_cmd=[]):
    network_info={}
    ex=[]
    try:
    ...
    for network in network_info.keys():
        ...
        # print cmd_show_network,controller_node
...
def check_in_host(compute_node,instance_name):
...
def check_vm(fuel_ip,vm_name,ex_cmd=[]):
    ...
    # print network
    print "check network"
    result["all_network_linked"],result["ex"]=check_network(network,controller_node,ex_cmd=ex_cmd)
    print "check host"
    result["virsh_status"],result["vm_xml"]=check_in_host(compute_node=result["host"],instance_name=result["instance_name"])
    return result


if __name__ == '__main__':
    r=check_vm(fuel_ip=fuel_ip,vm_name=vm_name,ex_cmd=["/root/cyborg_key.priv",["uname -a"]])
    print r
    logging.info(r)
