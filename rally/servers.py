...
@scenario.configure(context={"cleanup": ["nova"]},
                    name="NovaServers.boot_and_shutdown_start_reboot_delete_servers")
class Boot_Shutdown_Start_Reboot_DeleteServer(utils.NovaScenario, cinder_utils.CinderScenario):
    def sync_time(self,time_beg,priod):
        time_current = time.time()
        while time_current < time_beg + priod:
            time_current = time.time()
            time.sleep(1)
        return
